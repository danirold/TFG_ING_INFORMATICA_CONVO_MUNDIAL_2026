...
def main():
    print(f"=== INICIANDO FÁBRICA DE IA PARA TFG ===")
    ...
    resultados = {}
    todas_las_probabilidades = {} # <--- AQUÍ GUARDAREMOS LAS 12 LISTAS

    print("\nEntrenando 12 modelos (3 Escenarios x 4 Algoritmos)...")
    for v_nombre, X_vent, feats in [("v1_Meritocratico", X_v1, feats_v1), 
        ...
        for algo_nombre, modelo in modelos.items():
            ...
            todas_las_probabilidades[nombre_modelo] = y_prob # Guardamos la nota de cada jugador
            
    print("\n" + "="*50)
    print(" FASE 1: Resultados In-Sample (Auditoría Marzo 2026)")
    print("="*50)
    
    ruta_resultados_insample = os.path.join("datos_modelo", "resultados_fase1_insample.txt")
    with open(ruta_resultados_insample, "w", encoding="utf-8") as f_res:
        f_res.write("RESULTADOS FASE 1: PRECISION@26 (Ajuste a la convocatoria de marzo)\n")
        f_res.write("="*65 + "\n")
    ...
    print(f"\n¡ÉXITO! Los 12 modelos .pkl y los resultados In-Sample están guardados.")

    # =========================================================================
    # 5. GENERAR INFORME DE TEXTO CON LAS 12 CONVOCATORIAS
    # =========================================================================
    ...
    with open(ruta_informe, "w", encoding="utf-8") as f:
        f.write("="*80 + "\n")
        f.write(" FASE 2: PREDICCIONES OFICIALES PARA LA COPA DEL MUNDO 2026 (Out-of-Time)\n")
        f.write(" Evaluado con estadísticas actualizadas hasta el cierre de la base de datos.\n")
        f.write(" Estructura táctica: 3 Porteros | 8 Defensas | 8 Medios | 7 Delanteros\n")
        f.write("="*80 + "\n\n")

        # Recorremos los 12 modelos guardados
        for nombre_modelo, prob_array in todas_las_probabilidades.items():
            f.write(f"{'*'*75}\n")
            f.write(f" 🤖 MODELO: {nombre_modelo}\n")
            f.write(f"{'*'*75}\n")
            
            # Añadimos probabilidades al df
            ...
            # Añadimos la columna LIGA en la cabecera
            ...
            for i, (_, row) in enumerate(convocatoria_26.iterrows(), 1):
                ...
                # Limpiamos el nombre de la liga (quitamos 'es ', 'eng ', etc.)
                ...
                # Escribimos la fila con la nueva columna
                f.write(f"{i:<3} | {nombre:<22} | {pos:<3} | {equipo:<16} | {liga:<16} | {prob:>7.1%}\n")
            
            f.write("\n\n") # Espacio extra antes del siguiente modelo

    print("¡Informe generado con éxito! Ábrelo para comparar los resultados.")
